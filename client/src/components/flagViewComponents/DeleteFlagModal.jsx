...
const deleteConfirmationModal = ({ deletingFlag, setDeletingFlag, flag }) => {
	...
	const handleSubmit = () => {
		dispatch(
			deleteFlag(flag.id, () => {
				// is this necessary? we are routing to the dashboard and will never see this again?
				setDeletingFlag(false);
				props.history.push('/flags');
			})
		);
	};
					...
					<div className="space-y-8 divide-y divide-gray-200 sm:space-y-5 p-6">
						...
						<div className="clear-both py-10 mb-10">
							...
							<button
								...
								className="float-right inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md text-red-700 bg-red-100 hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
							>
								Nevermind, cancel.
							</button>
						</div>
